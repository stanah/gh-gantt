...
describe("[NFR-STABILITY-006-AC1] 要件とテストから生成する機能網羅レポート", () => {
  ...
  it("集計関数は 0 件の AC を 100% covered として扱わない", () => {
    ...
    expect(report.summary.acceptance_criteria.coverage_percent).toBe(0);
    expect(report.areas[0]?.acceptance_criteria.coverage_percent).toBe(0);
    expect(report.requirements[0]?.status).toBe("uncovered");
    expect(report.requirements[0]?.acceptance_criteria.coverage_percent).toBe(0);
  });
});
...
describe("[NFR-STABILITY-006-AC2] 要件網羅レポートは再生成で更新される", () => {
  ...
  it("不完全または重複した requirements.yaml を拒否する", async () => {
...
`,
      ...
      {
        name: "duplicate-acceptance-criteria-id",
        yaml: `version: "1.0"
vision: fixture
areas:
  - id: CLI
