...
describe("[FR-SYNC-004-AC1] GitHub Project Item をローカル Task 形式に変換できる", () => {
  ...
  it("[FR-CLI-012-AC2] 空の受入基準スロットを pull 後の task メタデータとして保持する", () => {
    const item = makeItem({
      content: {
        ...makeItem().content,
        body: [
    ...
    expect(task!.body).toBe("説明文");
    expect(task!.acceptance_criteria).toEqual([]);
    expect(task!.acceptance_criteria_slot).toBe(true);
  });

  it("[FR-CLI-013-AC3] Issue body のロールブロックを first-class フィールドへマッピングする", () => {
    const item = makeItem({
      content: {
        ...makeItem().content,
        body: [
    ...
    expect(task!.body).toBe("説明文");
    expect(task!.implementer).toBe("alice");
    expect(task!.reviewer).toBe("bob");
  });

  it("[FR-CLI-014-AC3] Issue body の review ブロックを first-class フィールドへマッピングする", () => {
    const item = makeItem({
      content: {
        ...makeItem().content,
        body: [
    ...
    expect(task!.review_approved_by).toBe("alice");
    expect(task!.review_approved_at).toBe("2026-05-03T21:00:00.000Z");
  });
