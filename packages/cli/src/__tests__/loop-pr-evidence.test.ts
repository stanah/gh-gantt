...
import type { PrGateState, PrGateTarget } from "../loop/pr-evidence.js";
import { completeIteration, formatLoopComplete } from "../commands/loop.js";

const NOW = "2026-07-07T10:00:00Z";

const REPO = { owner: "stanah", repo: "gh-gantt" };

const target = (number: number, overrides: Partial<PrGateTarget> = {}): PrGateTarget => ({
  ...REPO,
  number,
  crossRepo: false,
  ...overrides,
...
describe("[FR-CLI-018-AC7] extractLinkedPrTargets による PR 所在の列挙", () => {
  it("番号形式とメタデータ形式の混在から重複なしの昇順で PR 所在を列挙する", () => {
    const refs: LinkedPullRequestRef[] = [
      310,
      { number: 304, title: "PR", state: "open", url: null },
      310,
    ];
    expect(extractLinkedPrTargets(refs, REPO)).toEqual([target(304), target(310)]);
  });

  it("ローカルキャッシュの state には依存せず所在のみを使う", () => {
    ...
    expect(extractLinkedPrTargets(refs, REPO)).toEqual([target(1)]);
  });

  it("cross-repo の closing reference は URL からリポジトリを解決する", () => {
    const refs: LinkedPullRequestRef[] = [
      { number: 12, title: "PR", state: "open", url: "https://github.com/other/app/pull/12" },
    ];
    expect(extractLinkedPrTargets(refs, REPO)).toEqual([
      { owner: "other", repo: "app", number: 12, crossRepo: true },
    ]);
  });

  it("URL が欠落・解釈不能な場合はタスクのリポジトリとみなす", () => {
    const refs: LinkedPullRequestRef[] = [
      { number: 5, title: "PR", state: "open", url: null },
      { number: 6, title: "PR", state: "open", url: "https://example.com/not-a-pr" },
    ];
    expect(extractLinkedPrTargets(refs, REPO)).toEqual([target(5), target(6)]);
  });

  it("同一番号でもリポジトリが異なれば別の PR として列挙する", () => {
    const refs: LinkedPullRequestRef[] = [
      { number: 12, title: "PR", state: "open", url: "https://github.com/other/app/pull/12" },
      12,
    ];
...
describe("[FR-CLI-018-AC7] cross-repo PR の live 判定", () => {
  it("cross-repo の PR は evidence に repo を記録し、同一番号の別リポジトリ PR と区別する", () => {
    const crossTarget = target(12, { owner: "other", repo: "app", crossRepo: true });
    const result = evaluatePrEvidence({
      targets: [crossTarget, target(12)],
      fetched: [
        gateState({ owner: "other", repo: "app", number: 12, crossRepo: true }),
        gateState({ number: 12 }),
      ],
      checkedAt: NOW,
    });
    expect(result.kind).toBe("accepted");
    if (result.kind === "accepted") {
      expect(result.evidence[0].repo).toBe("other/app");
      expect(result.evidence[1]).not.toHaveProperty("repo");
    }
  });

  it("cross-repo の OPEN PR は診断表示にリポジトリ名を含める", () => {
    const openPr = gateState({
      owner: "other",
      repo: "app",
      number: 12,
    ...
    const result = evaluatePrEvidence({
      targets: [target(12, { owner: "other", repo: "app", crossRepo: true })],
      fetched: [openPr],
      checkedAt: NOW,
    });
    expect(result.kind).toBe("rejected_open_prs");
    if (result.kind === "rejected_open_prs") {
      expect(formatPrGateRejection(result)).toContain("PR other/app#12");
    }
  });
});

describe("[FR-CLI-018-AC3] --override-pr-gate の理由の正規化", () => {
  it("空文字・空白のみの理由は入力エラーとして拒否する", () => {
    expect(normalizeOverrideReason("")).toEqual({ ok: false });
    expect(normalizeOverrideReason("   ")).toEqual({ ok: false });
  });

  it("前後の空白を除去した理由を返し、未指定は undefined のまま通す", () => {
    expect(normalizeOverrideReason("  hotfix のため  ")).toEqual({
      ok: true,
      reason: "hotfix のため",
    });
    expect(normalizeOverrideReason(undefined)).toEqual({ ok: true, reason: undefined });
  });
});
