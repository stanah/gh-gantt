...
describe("[FR-SYNC-005-AC1] タスクの内容からハッシュを算出し変更を検出できる", () => {
  ...
  it("[FR-CLI-011-AC3] acceptance_criteria の変更を同期ハッシュへ反映する", () => {
    const task1 = {
      ...baseTask,
  ...
  it("[FR-CLI-012-AC2] acceptance_criteria_slot の変更を同期ハッシュへ反映する", () => {
    const task1 = { ...baseTask, acceptance_criteria_slot: false };
    const task2 = { ...baseTask, acceptance_criteria_slot: true };
    expect(hashTask(task1)).not.toBe(hashTask(task2));
  });

  it("[FR-CLI-013-AC3] implementer/reviewer の変更を同期ハッシュへ反映する", () => {
    const task1 = { ...baseTask, implementer: "alice", reviewer: "bob" };
    const task2 = { ...baseTask, implementer: "alice", reviewer: "carol" };
    expect(hashTask(task1)).not.toBe(hashTask(task2));
  });

  it("[FR-CLI-014-AC3] review requirement と approval の変更を同期ハッシュへ反映する", () => {
    const task1 = { ...baseTask, require_review: true, review_approved_by: "alice" };
    const task2 = { ...baseTask, require_review: true, review_approved_by: "bob" };
    expect(hashTask(task1)).not.toBe(hashTask(task2));
  });
});
