...
describe("computeLoopMetrics によるジャーナルメトリクス (ADR-016 案D)", () => {
  ...
  it("完了後に reopen されて失敗を繰り返すタスクは再び repeatedTasks に含まれる", () => {
    const metrics = computeLoopMetrics(
      state([
        // 一度完了した実績があっても、その後の停滞は抑止されない（完了でカウントがリセット）
        iteration({ selectedTask: "T-R", outcome: "completed" }),
        iteration({ selectedTask: "T-R", outcome: "verify_failed" }),
        iteration({ selectedTask: "T-R", outcome: "abandoned" }),
      ]),
    );
    expect(metrics.repeatedTasks).toEqual([{ taskId: "T-R", selections: 2 }]);
  });
