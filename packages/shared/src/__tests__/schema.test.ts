...
describe("[NFR-STORE-001-AC1] 不正な形式のファイルを読み込んだ場合にバリデーションエラーを返す", () => {
  ...
  it("[FR-STORE-001-AC4] field_mapping.status を含む config がバリデーションを通過する（後方互換）", () => {
    // validConfig は deprecated な field_mapping.status ("Status") を含む
    const parsed = ConfigSchema.parse(validConfig);
    expect(parsed.sync.field_mapping.status).toBe("Status");
  });

  it("[FR-STORE-001-AC4] field_mapping.status を含まない config もバリデーションを通過する", () => {
    const { status: _status, ...fieldMappingWithoutStatus } = validConfig.sync.field_mapping;
    const configWithoutStatus = {
      ...validConfig,
      sync: {
        ...validConfig.sync,
        field_mapping: fieldMappingWithoutStatus,
      },
    };
    const parsed = ConfigSchema.parse(configWithoutStatus);
    expect(parsed.sync.field_mapping.status).toBeUndefined();
  });
  ...
  it("[FR-VIS-025] grouping.facets を設定として受理する", () => {
    const config = {
      ...validConfig,
      grouping: {
        facets: [
    ...
    const parsed = ConfigSchema.parse(config);
    expect(parsed.grouping?.facets).toHaveLength(2);
  });

  it("[FR-VIS-025] grouping.facets の key が重複していれば拒否する", () => {
    const config = {
      ...validConfig,
      grouping: {
        facets: [
          { key: "system", label: "システム", label_prefix: "system:" },
          { key: "system", label: "重複", label_prefix: "sys:" },
        ],
      },
    };
    expect(() => ConfigSchema.parse(config)).toThrow();
