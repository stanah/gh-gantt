...
describe("[FR-VIS-007-AC1] 営業日計算・日付レンジ・スケジュールステータスを正しく算出する", () => {
  ...
  describe("[FR-VIS-016-AC1] 休日・休暇日を営業日計算から除外する", () => {
    it("平日でも休日・休暇日なら非稼働日として扱う", () => {
      const holidays = [{ date: "2026-01-06", name: "Company holiday" }];

      expect(isWorkingDay(new Date(2026, 0, 6), [1, 2, 3, 4, 5], holidays)).toBe(false);
    });

    it("休日・休暇日を飛ばして営業日を加算する", () => {
      const holidays = [{ date: "2026-01-06", name: "Company holiday" }];
      const result = addWorkingDays(new Date(2026, 0, 5), 1, [1, 2, 3, 4, 5], holidays);
  ...
  describe("schedule status helpers", () => {
    ...
    it("[FR-VIS-018-AC3] at-risk 閾値の未設定・不正値を既定値へフォールバックする", () => {
      expect(normalizeAtRiskThresholdDays(undefined)).toBe(3);
      expect(normalizeAtRiskThresholdDays(Number.NaN)).toBe(3);
      expect(normalizeAtRiskThresholdDays(Number.POSITIVE_INFINITY)).toBe(3);
      expect(normalizeAtRiskThresholdDays(0)).toBe(3);
      expect(normalizeAtRiskThresholdDays(-1)).toBe(3);
      expect(normalizeAtRiskThresholdDays(5)).toBe(5);
    });
  });
});
