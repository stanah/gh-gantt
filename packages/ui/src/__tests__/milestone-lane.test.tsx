...
describe("[FR-VIS-023-AC4] マーカーホバーでタイトル/日付がツールチップ用 aria-label として取得できる", () => {
  ...
  it("[FR-VIS-023-AC4] マーカーで Enter / Space を押すと onSelectTask が呼ばれる (キーボード操作)", async () => {
    ...
    const marker = container.querySelector('[role="graphics-symbol"]');
    expect(marker).not.toBeNull();
    fireEvent.keyDown(marker!, { key: "Enter" });
    fireEvent.keyDown(marker!, { key: " " });
    expect(onSelectTask).toHaveBeenCalledTimes(2);
    expect(onSelectTask).toHaveBeenCalledWith("M-1");
    // 関係ないキーでは選択されない
...
describe("[FR-VIS-023-AC5] マイルストーン位置からガント本体の全タスク行を縦に貫通する縦線が描画される", () => {
  ...
  it("enabledTypes で milestone type が無効化されている場合は縦線が描画されない (TypeFilter 連動)", () => {
    const taskA = baseTask({
      id: "T-1",
      type: "task",
      title: "Task1",
    ...
    const flatList: TreeNode[] = [{ task: taskA, children: [], depth: 0 }];
    const baseProps = {
      tasks: [taskA, milestoneTask],
      flatList,
      config,
      selectedTaskId: null,
      onSelectTask: () => {},
    ...
    // milestone を含む enabledTypes → 縦線あり
    const htmlEnabled = renderToStaticMarkup(
      <GanttChart {...baseProps} enabledTypes={new Set(["task", "milestone"])} />,
    );
    expect(htmlEnabled).toContain("data-milestone-vline");
    // milestone を除いた enabledTypes → 縦線なし
    const htmlDisabled = renderToStaticMarkup(
      <GanttChart {...baseProps} enabledTypes={new Set(["task"])} />,
    );
    expect(htmlDisabled).not.toContain("data-milestone-vline");
  });
});

describe("[FR-VIS-023] 左ヘッダーの高さがマイルストーンレーン分だけ補正される", () => {
  it("hasMilestoneLane=true のとき左ヘッダー高さにレーン分が加算され、右ペインのバーと縦位置が揃う", () => {
    const withLane = renderToStaticMarkup(
      <TaskTreeHeader config={config} hasMilestoneLane={true} />,
    );
    const withoutLane = renderToStaticMarkup(
      <TaskTreeHeader config={config} hasMilestoneLane={false} />,
    );
    // sprint バンドなし → ベース 32px。レーンありは 32 + MILESTONE_LANE_HEIGHT。
    expect(withoutLane).toContain("height:32px");
    expect(withLane).toContain(`height:${32 + MILESTONE_LANE_HEIGHT}px`);
  });
});
