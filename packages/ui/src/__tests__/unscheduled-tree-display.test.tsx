...
describe("[FR-VIS-015-AC1] 未スケジュール子タスクのツリー分類", () => {
  ...
  it("親タスクがフィルタで非表示の場合は未スケジュール子タスクを表示上の root として扱う", () => {
    const hiddenParent = makeTask("stanah/gh-gantt#1", {
      type: "epic",
      title: "非表示の親",
      sub_tasks: ["stanah/gh-gantt#2"],
    });
    ...
    function Probe() {
      const { flatList } = useTaskTree([hiddenParent, child], new Set(["task"]));
      return (
        <output>
          {flatList
            .map((node) => `${node.task.title}:${node.depth}:${node.scheduleState}`)
    ...
    expect(html).toContain("表示される未スケジュール子:0:unscheduled_root");
  });
});
...
describe("[FR-VIS-015-AC2] ツリー内未スケジュール子タスクの視覚表示", () => {
  ...
  it("TaskRow はツリー内未スケジュール子タスクに状態属性とラベルを表示する", () => {
    ...
    expect(html).toContain("border:1px dashed #27AE60;box-sizing:border-box");
