...
export function GanttGrid({ xScale, dateRange, totalWidth, totalHeight, workingDays, pixelsPerDay, sprints }: GanttGridProps) {
  ...
  const sprintBands = useMemo(() => {
    ...
    return sprints
      ...
      .filter((item): item is { key: string; start: Date; end: Date; color: string } => item != null);
  }, [dateRange, sprints]);
