...
Dependency Map は shared の `buildDependencySubgraph` が返す nodes / parentEdges / edges を入力に、レイアウトを dagre、描画を React Flow で行う（ADR-028）。プロジェクトの構造は Epic → Feature → Task の親子ツリーであり、ブロック関係はその上に重なる制約なので、**親子ツリーを骨格にし、ブロック関係を重ね書きする**。

- **ノード集合**: shared の `buildDependencySubgraph` が親子ツリーを基準に決める。「全依存」では親か子かブロック関係のいずれかを持つ全タスクがノードになる。どれも持たない孤立タスクは `isolatedTaskIds` に記録するだけでノードには含めず、ヘッダに件数と「孤立 N」トグルを出す (トグルで `includeIsolated` を有効にすると孤立タスクも 1 ノードの成分として描く)。「選択中心」では選択タスクの部分木 (子孫) を `selected`、部分木に対する上流 / 下流のブロック関係 2 階層を `upstream` / `downstream`、選択タスクからルートまでの親の連鎖を `ancestor` として含める。祖先は文脈として置くだけで、祖先からブロック関係は辿らない。`parentEdges` と `edges` はいずれも両端がノード集合に含まれる組だけを持つ。親は `parent` フィールドで解決し、未設定でも他タスクの `sub_tasks` に載っていれば親とみなす。
- **階層配置**: 親子辺だけを dagre に渡し、`rankdir: LR` で親を左、子を右に置く。同じ親を持つ子は同じ段に縦に積まれ、dagre が交差を減らす順に並べる。親子辺は 1 段しか跨がないため dagre の経路点は使わず、親の右辺中央と子の左辺中央を直線で結ぶ (親子の循環で子が左に置かれた逆向きの辺は親の左辺から子の右辺へ結び、ノード本体を貫通させない)。ノードは幅 220px の横長なので、段が浅く同じ段にノードが多いツリーでは縦向き (TB) だと極端に横長になるため、横向きにしている。ノードのハンドルは左辺 (target) と右辺 (source) にある。
- **ブロック辺の重ね書き**: ブロック関係は段付けに使わない。配置確定後に from の右辺中央から to の左辺中央へ 3 次ベジェで結び、制御点は始点の右・終点の左に水平距離 (両端の x 差の半分、最低 40px) だけ離して置く。to が from より左にある逆向きの辺 (同じ段の兄弟間や循環) でも同じ規則で、始点から右へ出て回り込み終点へ左から入る S 字になるため、向きが読めて経路が破綻しない。ブロック辺は dagre に渡していないので、ブロック辺同士の交差は減らさない。
- **連結成分の分割**: 親子とブロックの両方の辺で連結成分を求め (ブロックで繋がるツリー同士が離れないようにする)、成分ごとに dagre を実行してノード数の多い成分から順に縦に積み、左端 (ルートの段) を揃える。LR 配置では横軸が「左 = 親、右 = 子」を意味するため、無関係な成分を横に並べると左の成分の続きに読めてしまう。縦長になる分はパン・ズームで閲覧し、成分同士は段間隔より広い間隔で区切る。
- **ranker**: dagre の `ranker` は `network-simplex` (dagre の既定) を採用する。#370 で gh-gantt 自身の実データ (201 タスク、依存 38 件) をブロック関係の骨格で比較した結果は次の通り (親子ツリーの骨格に移行した #398 では、単一親のツリーなので ranker による段の差は生じない)。交差数は `countEdgeCrossings` (ノード境界中央を結ぶ直線同士の交差数)、縦横比は外接領域の幅 / 高さ。
...
- **エッジの表現**: 関係の種類を色と線種・線幅・形の組み合わせで区別する。定義は `DependencyMapPanel.tsx` の `dependencyEdgeStyles` の 1 箇所にまとめ、描画と凡例で共用する。色だけに頼らず線種と線幅を併用するため、色覚特性があっても、また `gantt.colors.critical_path` が danger と同じ赤に設定されていても区別できる。ブロック辺の基本線幅は 2px（クリティカルパスは 3.5px）で、ライト / ダーク両テーマの背景から浮く。親子は骨格として背景に退く細い muted の直線で、曲線のブロック辺とは形でも区別できる。

  | 種類                | 色                           | 線種            | 線幅  |
  | ------------------- | ---------------------------- | --------------- | ----- |
  | 親子                | `--color-text-muted`         | 実線 (直線)     | 1.5px |
  | ブロック (解決済み) | `--color-text-secondary`     | 実線 (曲線)     | 2px   |
  | ブロック (未解決)   | `--color-danger`             | 破線 (曲線)     | 2px   |
  | クリティカルパス    | `gantt.colors.critical_path` | 太い実線 (曲線) | 3.5px |

  未解決かつクリティカルパス上のエッジは critical_path 色の太い破線になり、両方の情報を保つ。ノードの左バーと枠線は readiness 列の色に従う。

- **依存タイプと lag**: 線種は関係の種類に使うため、`blocked_by` の `type` と `lag` はベジェ経路の中点 (t = 0.5) にラベルで示す。finish-to-start は既定なので略号を出さず、それ以外は `SS` / `FF` / `SF`、lag が 0 以外なら `+3d` のように添える（例: `FF -2d`）。
- **ブロックトグル**: 親子辺は構造線として常時表示し、ブロック辺はヘッダの「ブロック」トグルで表示 / 非表示にする（既定は表示）。非表示にしても段付けは親子辺で決まっているため、ノードの座標は変わらない。
- **凡例**: キャンバス左下に関係種別ごとの線見本とラベル、依存タイプ / lag ラベルの読み方を表示する。ブロック辺が非表示のときは凡例のブロック系統の行を薄くして「(非表示)」と示す。
- **ヘッダの hint**: 「N ノード / 親子 N / ブロック N」の形で、孤立タスクを非表示にしていれば「孤立 N 件非表示」、フィルタで除外があれば「フィルタで N 件非表示」を添える。
...
- **表示範囲**: ヘッダの「全依存 / 選択中心」トグルで切り替える。既定は「全依存」で、親か子かブロック関係を持つ全タスクを表示し、選択はノードの強調にだけ使う。「選択中心」では `buildDependencySubgraph` により選択タスクの部分木と、それに対するブロック関係 2 階層、選択タスクの祖先の連鎖に絞り込む (選択がなければ全体を表示する)。
...
- **フィルタ**: ツールバーのフィルタに一致しないタスクは shared の `pruneDependencySubgraph` でサブグラフから取り除く。除外ノードに接続していたブロック関係と親子関係は残ったノード側に上流 / 下流の件数として記録し (親は上流側、子は下流側として数える)、ノード上の省略記号で途切れを示す（7 章）。孤立タスクの件数もフィルタ後の表示対象に絞る。
