...
- セッション開始時・完了報告前の `--all-open` は current user / current branch ではなく
  repository の open PR 全件を対象にする。ユーザーが特定 PR だけを明示した場合を除き、
  open PR が残っているなら全て確認する。
...
- 完了報告前に repository の open PR 全件を sweep するため、current branch PR だけを
