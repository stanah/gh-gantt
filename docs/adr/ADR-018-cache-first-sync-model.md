...
`tasks.json` / `sync-state.json` は構造上すでにキャッシュに近く、`status`
（#303）と milestone `date`（#298）の pull-only ギャップを埋めれば「唯一の
データ」として扱う必要がなくなる。

## Decision

GitHub を唯一の真実源 (single source of truth) とし、`.gantt-sync/` 配下の同期
データ (`tasks.json`, `sync-state.json`, `loop-state.json`) を「いつ破棄しても
`gh-gantt pull` で再構築できる write-through キャッシュ」と再定義する。
...
- 既存の 3-way merge・コンフリクトマーカー・resolve コマンド (ADR-001) の
  ...
  検出・解決の仕組み自体ではない。
- レビュー時の fact-check により、push が ProjectV2 の Status フィールドを
  一度も更新しない既存バグ（pull-only の一方向）が判明した。本 ADR のスコープ
  外の独立した不具合として #303 で追跡し、`gh-gantt-workflow` の完了条件には
  含めない。ただし #303 が未解決のまま write-through push (#297) や
  git-common-dir 化 (#299) を進めると、`status` の変更だけがキャッシュ破棄で
  消失し続ける点に留意する。
