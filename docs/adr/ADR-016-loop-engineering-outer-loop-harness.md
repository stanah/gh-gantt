...
| 内側ループ + 停止条件       | dev-role の `maxExecutorRetries`（検証失敗）/ `maxImprovementIterations`（レビュー指摘） |
...
- このファイルは `tasks.json` / `sync-state.json` と同様に**直接編集禁止**とし、
  `gh-gantt loop` コマンド経由でのみ読み書きする（AGENTS.md の同期データ規約に従う。`CLAUDE.md` は `AGENTS.md` へのシンボリックリンク）。
- `Config` に `loop` セクションを追加し停止条件を形式化する。
  ...
  loop:
    ...
    stopWhen:
      ...
      - budget_exhausted
    onVerifyFailure: retry # retry 時の上限は executor 用の maxExecutorRetries に揃える
  ```
...
- `gh-gantt-workflow` の外側ループ散文は、段階的に `gh-gantt loop` コマンド呼び出しへ
  置換していく。dev-role（内側ループ）は変更しない。`onVerifyFailure: retry` の
  リトライ予算は executor/`verifyCommands` 失敗を律する dev-role の
  `maxExecutorRetries` に揃える。reviewer の `request-changes` / `block` ループを
  律する `maxImprovementIterations` とは別予算であり、混同しない。
- `.gantt-sync/loop-state.json` が新たな直接編集禁止ファイルとなる。
  AGENTS.md（`CLAUDE.md` はそのシンボリックリンク）の同期データ規約にこのファイルを追記する。
