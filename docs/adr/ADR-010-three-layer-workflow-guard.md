...
- **PostToolUse (gh pr create)**: PR 作成直後に「PR 作成は完了ではなく
  レビュー監視の開始である」リマインダー (pr-review-cycle-wait.sh の実行、
  同一 PR への追加コミット対応、完了報告前の --all-open) を command + exit 2 で
  エージェントに注入する。
...
hooks は Claude Code 固有であり、hook を実行できない環境 (Codex 等) では
この層は効かない。その環境でも効く防衛線は ADR-019 の CLI 側ゲートである。
