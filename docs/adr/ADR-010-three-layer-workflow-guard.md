...
- **prompt 型の advisory チェックリスト (コミット前・PR 作成前) は廃止**:
  ...
  もともと gh-gantt-workflow / dev-role の手順 (`before_commit` 等のフック
  ポイント) と L1 (lefthook pre-push が CI 同等チェックを強制) で担保されて
  おり、旧 prompt hooks は一度も発火していなかったため防御力の低下もない。本 ADR の「機械検査不可能なものは
  prompt でリマインド」という設計前提は、prompt 型が per-command で
  発火できないという実仕様の制約により、この用途では成立しなかったと記録する。

### 2026-07-21 追補: PR scope-selection の現行契約 (#324)

2026-05-02 と 2026-07-08 の追補にある「オープン PR 全件確認」と、PostToolUse hook が
完了報告前の `--all-open` を注入する記述は、当時の運用を示す**歴史的な契約**であり、
現行契約ではない。[ADR-020](ADR-020-bounded-agent-workflow-scope.md) が ADR-013 の
scope-selection 部分だけを supersede したため、現在の default は現在タスクの PR を
`--pr <number>` または `--current-branch` で確認することとする。

リポジトリ全体の `--all-open` はユーザーが repository-wide audit を求めた場合の
明示的な opt-in とする。PostToolUse / Stop hook も現在タスクの PR の確認だけを促し、
`--all-open` を必須操作として注入しない。待機条件、fail-open、再入防止、hooks は
補助層であるというその他の決定は変更しない。
