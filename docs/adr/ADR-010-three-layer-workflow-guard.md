...
- PR review thread / checks / review decision の正本は GitHub PR であり、操作面は
  `gh pr view`, `gh pr checks`, `gh api graphql` とする。
- 完了保証は `.claude/hooks` ではなく、`skills/gh-gantt-workflow` が提供する
  `scripts/pr-review-cycle-wait.sh` を明示実行する。Codex など hook を自動実行できない
  agent でも同じ手順を使えるようにするためである。
- script は PR 作成後・push 後・セッション再開時に、未解決 review thread /
  changes requested / pending または blocking check / PR comments・reviews の quiet
  window を確認する。
- 詳細手順は `skills/gh-gantt-workflow/references/pr-review-cycle.md` に置く。gh-gantt
  CLI に薄い wrapper を追加しない。
- 対応結果は GitHub GraphQL の `addPullRequestReview` と
  `addPullRequestReviewThreadReply` で pending review に積み、
  `submitPullRequestReview` で 1 回だけ通知する。
- 対応済み thread の resolve は `resolveReviewThread` を GraphQL alias mutation として
  まとめて実行する。
