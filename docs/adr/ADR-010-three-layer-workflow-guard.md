...
- `.claude/settings.json` の `PostToolUse` と `UserPromptSubmit` は
  `.claude/hooks/pr-review-cycle-check.sh` を呼び、PR 作成後・push 後・次セッションで
  未解決 review thread / changes requested / 非 pass check を検出して対応を促す。
- 詳細手順は `skills/gh-gantt-workflow/references/pr-review-cycle.md` に置く。
  gh-gantt CLI に薄い wrapper を追加しない。
