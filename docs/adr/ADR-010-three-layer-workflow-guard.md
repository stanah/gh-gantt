...
- Claude Code hooks の `PostToolUse` と `UserPromptSubmit` から
  `gh-gantt review-cycle check` を実行し、PR 作成後・push 後・次セッション開始時に
  未解決 review thread と changes requested review を検出する。
- review thread の作者を bot / human に分類し、CodeRabbit / Copilot と人間レビューを
  同じ手順で扱う。
- 対応結果は `gh-gantt review-cycle submit --plan <json>` で pending review を作成し、
  `addPullRequestReviewThreadReply` で返信を積んだあと `submitPullRequestReview` で
  1 回だけ通知する。
