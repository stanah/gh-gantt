...
- .claude/settings.json がリポジトリにコミットされるため、Claude Code を 使わない開発者にとっては不要なファイルが増える (.gitignore 対象外)
- prompt タイプの hooks はエージェントへのリマインドであり完全な強制ではない。 エージェントが無視する可能性はゼロではないが、CLAUDE.md テキストと比べ ツール呼び出しの直前に毎回注入される点で実効性は大幅に向上する
- L1 と L2 でブランチ状態チェックが二重実行されるが、実行時間は無視できる程度 であり、防御の冗長性として許容する
