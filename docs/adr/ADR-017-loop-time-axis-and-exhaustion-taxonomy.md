...
### 1. decide は Next Actions スコアリングを再利用する

`gh-gantt loop next` の decide ステップは、readiness 単独ではなく
Project Map の Next Actions（FR-VIS-024）のスコアリング
（ready / 優先度 / 下流解除数 / クリティカルパス / リスク、安定ソート）を再利用し、
最上位のタスクを選定する。iteration plan には選定理由
