...
| From          | To                                       | 手段                   | 維持方法                  |
| ------------- | ---------------------------------------- | ---------------------- | ------------------------- |
| 要件 → テスト | `requirements.yaml` の `tests`, `status` | `req-trace` スクリプト | 自動更新                  |
| ADR → 要件    | ADR の `related_requirements`            | YAML フィールド        | 手書き (作成時、以後不変) |
| テスト → 要件 | テスト名の `[ID]` プレフィックス         | describe 文字列        | 手書き (テスト作成時)     |
...
- **Orphaned AC** (warning): `requirements.yaml` にあるがテストから参照されない AC。段階的導入のため warning 扱い
- **Orphaned Tag** (error): テスト名に ID があるが `requirements.yaml` に存在しない
- **Stale ADR Ref** (error): ADR の `related_requirements` が `requirements.yaml` に存在しない

注: Status Drift（`status` が実テスト結果と不一致）は `req:trace` が自動更新するため、`req:validate` では検出しない。CI で `req:trace` → `req:validate` の順に実行する前提。
