...
| 3 | `git push` | リモートが進んでいたら拒否 | `gh-gantt push` | ✅ | ✅ E2E |
| 4 | `git push --force` | リモートの状態を無視して強制 push | `gh-gantt push --force` | ✅ | ❌ (リモート変更チェックが updated_at ヒューリスティックのため検証困難) |
...
| 14 | pull でコンフリクトしたフィールドは resolve 後に push 可能 | resolve → snapshot 更新 → push で反映 | ✅ | ✅ E2E |
