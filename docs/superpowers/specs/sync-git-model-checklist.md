...
|---|---------|------|--------------|:----:|:-------:|
| 1 | `git pull` | いつでも実行可能。fetch + merge | `gh-gantt pull` | ✅ | ❌ |
| 2 | `git pull`（ローカル変更あり） | ローカル変更があっても merge 可能 | `gh-gantt pull`（ガードなし） | ✅ | ❌ |
| 3 | `git push` | リモートが進んでいたら拒否 | `gh-gantt push` | ✅ | ❌ |
| 4 | `git push --force` | リモートの状態を無視して強制 push | `gh-gantt push --force` | ✅ | ❌ |
| 5 | `git status` | ローカル変更の確認 | `gh-gantt status` | ✅ | ❌ |

## マージ・コンフリクト

| # | git 操作 | 説明 | gh-gantt 対応 | 実装 | 動作確認 |
|---|---------|------|--------------|:----:|:-------:|
| 6 | 3-way merge | base/local/remote で比較、非衝突は自動マージ | `threeWayMerge()` | ✅ | ❌ |
| 7 | コンフリクトマーカー (`<<<<`) | 衝突箇所をファイルに記録 | `{field}_current` / `{field}_incoming` キー | ✅ | ❌ |
| 8 | `git status`（コンフリクト中） | 未解決ファイルの一覧表示 | `gh-gantt conflicts` | ✅ | ❌ |
| 9 | エディタで解決 + `git add` | 衝突を手動解決 | `gh-gantt resolve --ours/--theirs` | ✅ | ❌ |
| 10 | コンフリクト未解決で merge 不可 | 解決するまで次の merge をブロック | `has_conflicts` ガードで pull ブロック | ✅ | ❌ |
| 11 | コンフリクト未解決で push 不可 | 解決するまで push をブロック | `has_conflicts` ガードで push ブロック | ✅ | ❌ |

## ローカル変更の保持

| # | git の動作 | gh-gantt の対応 | 実装 | 動作確認 |
|---|-----------|---------------|:----:|:-------:|
| 12 | pull 後もローカル変更は push 対象として残る | snapshot.hash を保持し push 差分が検出される | ✅ | ❌ |
| 13 | pull でコンフリクトしなかったローカル変更は push 可能 | 非衝突フィールドは自動マージされ push 対象として残る | ✅ | ❌ |
...
| # | git の動作 | gh-gantt の対応 | 実装 | 動作確認 |
|---|-----------|---------------|:----:|:-------:|
| 15 | リモートで削除 + ローカル未変更 → ローカルも削除 | タスクを削除 | ✅ | ❌ |
| 16 | リモートで削除 + ローカル変更あり → 警告して保持 | 警告表示しタスクを保持 | ✅ | ❌ |
