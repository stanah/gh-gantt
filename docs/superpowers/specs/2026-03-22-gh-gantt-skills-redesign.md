...
gh-gantt の `.claude/skills/` 配下のスキルは、gh-gantt CLI を使う任意のプロジェクトで適用できる汎用的な知識を提供する。
...
| `conflict-resolution` | 同期 | (既存) コンフリクト解決 |
...
digraph skills {
  ...
  "gh-gantt-sync" -> "conflict-resolution" [label="コンフリクト発生時"];
...
**責務:**
- pull/push の実行と結果の検証
- コンフリクト検出時に `conflict-resolution` へルーティング
...
失敗時: `conflict-resolution` スキルを invoke する。解決完了まで他の作業に進まない。
...
**プロセス:**
1. `gh-gantt task list` で全タスクと依存関係を取得
...
**プロセス:**
1. **REQUIRED:** `gh-gantt-sync`（pull）を invoke して最新データを取得
2. `gh-gantt task list` + `gh-gantt status` で全体像把握
3. 分析・レポート
4. アクションの提案（タスク着手、日程調整、ブロッカー解消等）

---

#### `conflict-resolution`（既存）
...
スキルファイルは `skill.md`（小文字）に統一する。既存の `conflict-resolution/SKILL.md` は `skill.md` にリネームする。
...
2. `conflict-resolution/SKILL.md` → `skill.md` リネーム
