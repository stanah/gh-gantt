...
**Architecture:** 各スキルは `.claude/skills/<name>/skill.md` に配置。共通のコマンドリファレンスは `references/commands.md` に集約し、各スキルからシンボリックリンクまたはパス参照する。スキル間は REQUIRED/OPTIONAL チェーンで接続。
...
**Files:**
- Rename: `.claude/skills/conflict-resolution/SKILL.md` → `.claude/skills/conflict-resolution/skill.md`
- Rename: `.claude/skills/gh-gantt-workflow/SKILL.md` → `.claude/skills/gh-gantt-workflow/skill.md`
- Create: `.claude/skills/gh-gantt-sync/`
- Create: `.claude/skills/gh-gantt-decompose/`
- Create: `.claude/skills/gh-gantt-triage/`
- Create: `.claude/skills/gh-gantt-dependencies/`
- Create: `.claude/skills/gh-gantt-progress/`

- [ ] **Step 1: conflict-resolution のリネーム**

```bash
cd /Users/stanah/work/github.com/stanah/gh-gantt
git mv .claude/skills/conflict-resolution/SKILL.md .claude/skills/conflict-resolution/skill.md
```
...
```bash
git mv .claude/skills/gh-gantt-workflow/SKILL.md .claude/skills/gh-gantt-workflow/skill.md.tmp
git mv .claude/skills/gh-gantt-workflow/skill.md.tmp .claude/skills/gh-gantt-workflow/skill.md
```

- [ ] **Step 3: 新スキルのディレクトリ作成**

```bash
mkdir -p .claude/skills/gh-gantt-sync
mkdir -p .claude/skills/gh-gantt-decompose
mkdir -p .claude/skills/gh-gantt-triage
mkdir -p .claude/skills/gh-gantt-dependencies
mkdir -p .claude/skills/gh-gantt-progress
```
...
**Files:**
- Modify: `.claude/skills/gh-gantt-workflow/skill.md`
...
```bash
git add .claude/skills/gh-gantt-workflow/skill.md
...
**Files:**
- Create: `.claude/skills/gh-gantt-sync/skill.md`
...
```bash
git add .claude/skills/gh-gantt-sync/
...
**Files:**
- Create: `.claude/skills/gh-gantt-decompose/skill.md`
...
```bash
git add .claude/skills/gh-gantt-decompose/
...
**Files:**
- Create: `.claude/skills/gh-gantt-triage/skill.md`
...
```bash
git add .claude/skills/gh-gantt-triage/
...
**Files:**
- Create: `.claude/skills/gh-gantt-dependencies/skill.md`
...
```bash
git add .claude/skills/gh-gantt-dependencies/
...
**Files:**
- Create: `.claude/skills/gh-gantt-progress/skill.md`
...
```bash
git add .claude/skills/gh-gantt-progress/
...
```bash
grep -r "REQUIRED.*invoke\|OPTIONAL.*invoke" .claude/skills/*/skill.md
```

- [ ] **Step 3: references/commands.md へのパス参照が有効であることを確認**

```bash
grep -r "references/commands.md" .claude/skills/*/skill.md
```
