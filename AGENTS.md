...
- **`gh-gantt-pr`** — Issue から branch 名と PR description を標準化して `gh pr create` を実行する手順
...
# 単一パッケージのテスト
pnpm --filter gh-gantt test
pnpm --filter @gh-gantt/shared test
pnpm --filter @gh-gantt/ui test

# 単一テストファイル
pnpm --filter gh-gantt exec vp test run src/__tests__/hash.test.ts

# CLI をグローバルにリンク（初回 or 再ビルド後）
pnpm build && pnpm --filter gh-gantt exec pnpm link --global
```
...
- **`@gh-gantt/shared`** — 型定義と Zod スキーマ（`private`、`gh-gantt` パッケージに bundle されて配布される）
- **`gh-gantt`** — CLI (Commander) + REST API (Express) + 同期エンジン + GitHub GraphQL クライアント。npm に publish される唯一のパッケージ
- **`@gh-gantt/ui`** — React SPA (Vite Plus + D3)。ビルド成果物が `gh-gantt` の dist に同梱され、`gh-gantt serve` から配信される
