...
Issue 番号などで既知の単一タスクが指定されている場合は、その Issue の body / 受入基準と
関連する commit / diff に scope を限定してよい。この分岐ではリポジトリ全体の git log は必須ではない。

閉じ忘れの探索など複数タスクを対象にする横断監査の場合だけ、次のコミット範囲を確認する。
...
3. 変更内容とオープンタスクのタイトルを照合し、関連するタスクを特定する
   - 必要に応じて `gh-gantt list --search <キーワード> --json | node skills/gh-gantt-workflow/scripts/project-task-list-evidence.mjs` で bounded な候補を検索する。helper の `--limit <n>` / `--status-field <name>` と共通ステップの優先順位を維持する
   - Grep / Glob / Read / LSP でコードベースを補足調査する
4. 関連が見つかったオープンタスクについて `gh-gantt show <id>` で body を読む
