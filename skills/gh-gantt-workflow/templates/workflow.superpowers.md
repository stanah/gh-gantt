...
3. **対応後の再検証**（修正規模により段階的）:
   - **軽微な修正**（typo、コメント、フォーマット、変数名変更、lint 対応のみ）: 基準 1〜3（lint / typecheck / test）の再実行 + 基準 5（ユーザー承認）のみでよい。基準 4（外部レビュー）は省略可
   - **実質的な変更**（ロジック修正、設計変更、新規コード追加）: `before_commit` のフル 5 基準を再度通す
...
| レビュー基準を 4/5 で満たして commit する | ハード閾値は全基準必須。1 つでも落ちたら失敗 |
...
- **スクリプト**（`package.json` の scripts 名）:
  - テスト (JSON reporter): `<script name>`（例: `test:json`）
  - Reconciliation: `<script name>`（例: `req:trace`）
  - 整合性検証: `<script name>`（例: `req:validate`）
  - 自動生成ドキュメント: `<script name>`（例: `docs:gen`）

振る舞い変更を伴う開発では、`gh-gantt-living-documentation` スキルを invoke して要件 AC の追加とテスト名への `[ID]` 付与を行うこと。Reconciliation 実行も `before_commit` フック内で行う。
