...
5. 対応結果は `gh-gantt review-cycle submit --plan <json>` で pending review に返信を積み、submit で 1 回だけ通知する
6. 対応済み thread は同じ plan の `resolveThreadIds` に入れ、GraphQL alias mutation で一括 resolve する
