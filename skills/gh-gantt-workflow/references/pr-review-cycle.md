...
- セッション開始時: `skills/gh-gantt-workflow/scripts/pr-review-cycle-wait.sh --all-open --no-wait`
...
`--all-open` は current user や current branch ではなく、repository の open PR 全件を確認する。
ユーザーが「#195 だけ」のように明示した場合を除き、open PR が残っているなら全て確認対象にする。
...
- CodeRabbit の rate limit comment が現在も active かどうか。issue comments 取得は activity 取得と
  1 回にまとめ、check 表示より最新の CodeRabbit comment を優先する。
...
4. 完了報告前に `skills/gh-gantt-workflow/scripts/pr-review-cycle-wait.sh --all-open --no-wait`
   を実行し、repository の open PR 全件を列挙する

current branch の PR が clean でも、別の open PR に未解決レビューが残っていれば
レビューサイクル全体は完了していない。完了報告では確認した PR 番号と未解決 thread 件数を
必ず列挙する。
...
- 完了報告前の `--all-open --no-wait` で repository の open PR 全件を確認し、
  未解決が残る PR を隠していない
...
| current branch の PR だけを見て完了報告する | 別の open PR に残った未解決 thread を見落とす     |
