...
4. 完了報告前に `skills/gh-gantt-workflow/scripts/pr-review-cycle-wait.sh --all-open --no-wait`
   を実行し、リポジトリのオープン PR 全件を列挙する

現在ブランチの PR が clean でも、別のオープン PR に未解決レビューが残っていれば
...
- 完了報告前の `--all-open --no-wait` でリポジトリのオープン PR 全件を確認し、
  未解決が残る PR を隠していない
...
| 現在ブランチの PR だけを見て完了報告する  | 別のオープン PR に残った未解決 thread を見落とす  |
