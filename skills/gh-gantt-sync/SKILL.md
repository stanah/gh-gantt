---
name: gh-gantt-sync
description: gh-gantt の pull/push 同期を実行する。「同期して」「pull して」「push して」で使用。コンフリクト発生時は gh-gantt-conflict-resolution にチェーンする。他スキルからの REQUIRED チェーンでも発動。
---
...
失敗時: **REQUIRED:** `gh-gantt-conflict-resolution` スキルを invoke する。解決完了まで他の作業に進まない。
