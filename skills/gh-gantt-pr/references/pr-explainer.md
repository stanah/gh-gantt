...
- 単一ファイル。CSS / JS はインライン、画像は data: URI で埋め込む。外部 CDN / Web font / 外部画像だけでなく、
  相対パスや絶対パスの別ファイル参照も置かない。artifact の直接表示は単一ファイルだけを配信するため、
  別ファイルへの参照は種類を問わず描画されない
...
1. HTML の単一ファイル契約（HTML 文書であること、data: URI 以外の別ファイル参照がないこと）を検証する
...
| 制約         | 内容                                                                                                          |
| ------------ | ------------------------------------------------------------------------------------------------------------- |
| 閲覧条件     | artifact URL はリポジトリに read 権限を持つログイン済みユーザーだけが開ける。公開リポジトリでもログインは必要 |
...
4. HTML が必要なら、出力先ディレクトリに単一 HTML を書く（別ファイル参照なし、実データ、日本語）
...
| 外部 CDN、Web font、相対パスの画像や CSS を HTML から参照する | 単一ファイルしか配信されないため描画されない                            |
