...
| 2. ADR                   | 意思決定記録 (Markdown + frontmatter)   | 書いたら不変                 | 手書き             |
...
1. `<ADR_DIR>` の末尾 +1 で連番を振る（ファイル名は `ADR-<NNN>-<kebab-slug>.md`）
2. frontmatter 付き Markdown でテンプレートに従って作成
3. **代替案と却下理由を必ず書く**: 「rationale は却下された選択肢にこそある」(Martraire)
4. `related_requirements` で関連する要件 ID を参照（不要なら省略）
5. コミット
...
### Issue body の `## 完了条件` と 要件 AC は別概念

似た言葉だが混同しないこと。Issue body の `## 完了条件` は **このタスクを完了するための作業チェックリスト**で `- [ ]` 形式で書く（規約は `gh-gantt-decompose` skill 参照）。要件 AC (`acceptance_criteria`) は **プロダクトが満たすべき不変の振る舞い契約**で、Issue close では消えない。

|              | Issue body の `## 完了条件` (`- [ ]`)           | 要件 AC (`requirements.yaml`)                                    |
| ------------ | ----------------------------------------------- | ---------------------------------------------------------------- |
| **性質**     | このタスクの作業項目                            | プロダクトの振る舞い契約                                         |
| **寿命**     | Issue close で完了                              | プロダクト存続中は不変                                           |
| **粒度**     | タスク内の細かい作業                            | プロダクト全体の振る舞い                                         |
| **例**       | 「`X` コマンドが動く」「テスト追加」「PR 作成」 | 「ローカル・リモート両方が変更した場合にコンフリクトを検出する」 |
| **検証手段** | チェックボックスを ✓                            | 対応テストが covered になる (`req:trace`)                        |
